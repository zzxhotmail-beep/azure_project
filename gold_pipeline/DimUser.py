...
def dimuser_stag():
    df = spark.readStream.table("spotify_cata.silver.dimuser")      #直接读取 Databricks 内已存在的表
    return df           #把读取到的数据流返回，DLT 自动将其落地为 dimuser_stag 这张 DLT 表。


dlt.create_streaming_table(name="dimuser") #提前定义一张空的流式 Delta 表 dimuser，作为 CDC/SCD 的最终目标表；

dlt.create_auto_cdc_flow(
    target="dimuser",               #要写入的目标表名
    source = "dimuser_stag",        #CDC 数据来源
    keys= ["user_id"],              #主键 用来判断 “同一条记录”
